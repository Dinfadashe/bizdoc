...
'    // Show autocomplete suggestions when typing in description',
...
'      );',
'      if (matches.length > 0) {',
'        setSuggestions({ itemId: id, matches });',
'      } else {',
'        setSuggestions(null);',
'      }',
...
'    setItems(items.map(i => i.id === lineItemId ? {',
'      ...i,',
...
'              <button onClick={handleCancelDuplicate} style={{ flex: 1, padding: "11px", background: "#f5f2ed", color: "var(--text)", border: "1.5px solid var(--border)", borderRadius: 8, fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "var(--font-body)" }}>Cancel</button>',
...
'                            <div',
'                              key={match.id}',
'                              onMouseDown={() => applyCatalogItem(item.id, match)}',
...
'                              </div>',
'                              <div style={{ fontWeight: 700, fontSize: 13, color: "var(--green)", marginLeft: 16 }}>',
'                                {Number(match.unit_price).toLocaleString()}',
'                              </div>',
'                            </div>',
'                          ))}',
'                        </div>',
