...
export default function EditInvoice() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (!inv || inv.user_id !== user.id) { setNotFound(true); setLoading(false); return; }
      if (inv.status !== "draft") { router.push(`/invoices/${id}`); return; }

      // Pre-populate form
      setCurrency(inv.currency ?? "NGN");
      setClient({ name: inv.client_name ?? "", email: inv.client_email ?? "", phone: inv.client_phone ?? "", address: inv.client_address ?? "" });
      ...
      setItems(inv.items.map((it: { description: string; qty: number; unit_price: number }, i: number) => ({
        id: i + 1, description: it.description, qty: it.qty, unit_price: String(it.unit_price),
      })));

      const [catalogRes, accountsRes] = await Promise.all([
        supabase.from("catalog").select("*").eq("user_id", user.id).order("name"),
        fetch("/api/business-accounts?user_id=" + user.id),
      ]);
      setCatalog(catalogRes.data ?? []);
      const acctData = await accountsRes.json();
      const accounts = acctData.accounts ?? [];
      setBizAccounts(accounts);
      if (inv.display_account_id) {
        setSelectedAccountId(inv.display_account_id);
      } else {
        const def = accounts.find((a: BizAccount) => a.is_default) || accounts[0];
  ...
  const updateItem = (itemId: number, field: keyof LineItem, val: string | number) => {
  ...
  const handleSave = async () => {
    ...
    const res = await fetch(`/api/invoices/${id}`, {
      ...
      body: JSON.stringify({
        ...
        notes, issue_date: issueDate, due_date: dueDate,
        currency,
  ...
  if (notFound) return <div style={{ padding: 60, textAlign: "center", color: "#888" }}>Invoice not found.</div>;
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--cream)" }}>
      <nav style={{ background: "var(--green)", padding: "0 28px", height: 60, display: "flex", alignItems: "center", gap: 16 }}>
        <button onClick={() => router.push(`/invoices/${id}`)} style={{ background: "none", border: "none", color: "white", cursor: "pointer", fontSize: 20 }}>&#8592;</button>
        <div style={{ fontFamily: "var(--font-display)", fontSize: 20, fontWeight: 700, color: "white" }}>Edit Draft Invoice</div>
      </nav>
      ...
      <div style={{ maxWidth: 860, margin: "0 auto", padding: "28px 20px" }}>
        ...
        <div style={{ background: "white", borderRadius: 12, border: "1px solid var(--border)", marginBottom: 16, overflow: "hidden" }}>
          ...
          <div style={{ padding: 20, display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 14 }}>
            <div><label style={lbl}>Client Name</label><input value={client.name} onChange={e => setClient({ ...client, name: e.target.value })} style={inp} placeholder="Client or company name" /></div>
              ...
              <tbody>
                {items.map(item => (
                  <tr key={item.id}>
                    ...
                    <td style={{ padding: "7px 6px" }}><button onClick={() => items.length > 1 && setItems(items.filter(i => i.id !== item.id))} style={{ background: "none", border: "none", cursor: "pointer", color: "#ccc", fontSize: 20 }}>×</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
          <button onClick={() => router.push(`/invoices/${id}`)} style={{ padding: "12px 24px", background: "#f5f2ed", color: "var(--text)", border: "1.5px solid var(--border)", borderRadius: 8, fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "var(--font-body)" }}>Cancel</button>
          <button onClick={handleSave} disabled={saving} style={{ padding: "12px 28px", background: "var(--green)", color: "white", border: "none", borderRadius: 8, fontSize: 14, fontWeight: 700, cursor: saving ? "not-allowed" : "pointer", opacity: saving ? 0.7 : 1, fontFamily: "var(--font-body)" }}>{saving ? "Saving..." : "Save Changes"}</button>
        </div>
      </div>
    </div>
  );
