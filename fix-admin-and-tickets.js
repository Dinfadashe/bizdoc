...
// ── 1. Fix case-sensitive email search in admin API ──────────
let adminContent = fs.readFileSync('app/api/admin/route.ts', 'utf8');
adminContent = adminContent.replace(
  'const user = users?.users?.find((u: any) => u.email === email);',
  'const user = users?.users?.find((u: any) => u.email?.toLowerCase() === email?.toLowerCase().trim());'
);
fs.writeFileSync('app/api/admin/route.ts', adminContent, 'utf8');
console.log('Fixed case-sensitive email search');

// ── 2. Support tickets API ───────────────────────────────────
fs.mkdirSync('app/api/support', { recursive: true });
fs.writeFileSync('app/api/support/route.ts', `
...
`.trim(), { encoding: 'utf8' });
console.log('Created support API');

// ── 3. User support page ─────────────────────────────────────
fs.mkdirSync('app/support', { recursive: true });
fs.writeFileSync('app/support/page.tsx', `"use client";
